...
def reupload(tweet):
	...
	for med in tweet._json['extended_entities']['media']:
		if med['type'] == 'photo':
			...
			get_index_video_url = -1 if med['video_info']['variants'][-1]['content_type'] == 'video/mp4' else -2
			media_url = med['video_info']['variants'][get_index_video_url]['url']		
	...
	if media_paths:
		if Mongo.my_tweet_collection.find_one({"media_paths": json.dumps(media_paths)}):
			print("Tweet duplicate")
			return
		update_status = twit.update_status_media_upload(status, media_paths, media_type)
		if update_status is not None:
			if video_dl:
				twit.api.update_status(status=f"Download disini {video_dl}", in_reply_to_status_id=update_status.id_str, auto_populate_reply_metadata=True)
			my_tweet_insert = {
